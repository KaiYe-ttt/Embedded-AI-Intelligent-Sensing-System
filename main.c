...
/**
 * 完善后的滑动窗口函数（保留原有循环存储，新增有序序列整理）
 * 1. 保留你原有的循环写入逻辑
 * 2. 新增：整理出「从最旧到最新」的有序时间序列，供LSTM直接使用
 * 3. 新增：窗口填满标识，区分未填满/已填满场景
 * @param new_input  新的3维量化输入数据
 * @param ordered_window  输出：整理后的有序时间序列（最旧→最新，LSTM可直接使用）
 */
